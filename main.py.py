...
def genetic_algorithm(c_limit, p_size, gen_size, mutation_rate, i_list):
    ...
    best_solutions = []  # List to hold the best solution from each generation
    ...
    for i in range(gen_size):
        ...
        for j in range(len(pop)):
            gen_val.append(calculate_value(i_list, pop[j]))

        print(f"Values --> {gen_val}")
        current_best = best_solution(pop, i_list)
        best_solutions.append(current_best)  # Add the best value of the current generation
        value_list.append(current_best)  # This also holds the best values for plotting

    print('\nBest solutions List :', best_solutions)
